...
import { FlightOfferRepositoryError } from "./flight-offer-repository";
...
/**
 * The single deterministic demonstration-offer generator.
 *
 * Extracted in V2.7 so there is exactly one implementation behind both the
 * server-side provider adapter (`local-deterministic-provider-adapter.ts`)
 * and the deterministic verification scripts. Nothing here is a provider
 * integration: every price, time and identifier is a pure function of the
 * normalized Search Intent and an offer index, with no network call, no
 * environment variable, no clock read and no randomness that is not seeded
 * from the intent itself.
 *
 * This module is intentionally environment-neutral — it is pure TypeScript
 * with no server-only or browser-only API — but in the V2.7 runtime it is
 * only ever executed on the server, behind the provider adapter.
 */
/**
 * Identities come from the shared demonstration catalog, never from a literal
 * here. The canonical validator checks every generated identity against that
 * same catalog, so a name invented locally in this file would be rejected by
 * the boundary rather than quietly shipped.
 */
const CARRIERS = DEMO_CARRIERS;
const PROVIDERS = DEMO_BOOKING_PROVIDERS;
 ...
 * Return date without violating chronology (see the shared
 * `MIN_ROUND_TRIP_TURNAROUND_MINUTES` in `flight-offer-policy.ts`, which the
 * canonical validator enforces too) is skipped rather than forced, so the
 * candidate pool is generated
...
/**
 * The bounded candidate scan that produces one deterministic offer set.
 *
 * At most `MAX_OFFER_CANDIDATES` offer indices are ever built — bounded,
 * never unbounded — and any index whose round trip cannot honor chronology
 * (see `buildOffer`) is skipped rather than forced. The target count is a
